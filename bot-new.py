...
### ---- Layout ---- ###
    ...
    ####  VICE PRESIDENT & CHAIRMAN  ####
        ...
        if count == 0:
            st.error(f"Tables still empty.")
        else:
            agree = st.checkbox("Vote for Vice-President & Vice-Chairman")
            if agree:
                ### TEST IF PENDING TIE IS NOT YET RESOLVED IN PRESIDENT & CHAIRMAN ###
    ...
    #### MEMBER 2
        ...
        if agree:
            ...
            if win_shape > 1:
                ...
                col1,col2 = st.beta_columns(2)
                with col1:
                    member2_button = col1.button("VOTE for Member 2")
                    if member1_button:
                        add_vote(member2[0],member1_title,member2[1],datetime.date(datetime.now()))
                        col1.success(f"Vote successful.")
                with col2:
                    col2.warning(f"Select nominee then click **Submit Vote** once.")
